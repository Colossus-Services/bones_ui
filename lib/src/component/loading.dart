...
UILoadingType? getUILoadingType(Object? type) {
  ...
  switch (typeStr) {
    ...
    // Note: `typeStr` is lower-cased, so it can't be matched as `dualRing`:
    case 'dualring':
    case 'dual-ring':
    case 'dual_ring':
...
class UILoadingConfig implements AsDOMElement {
  ...
  static UILoadingConfig? fromMap(Map attributes, [String? prefix]) {
    ...
    // Accepts both the attribute style (`text-zoom`) and the
    // inline properties style generated by [toInlineProperties] (`textZoom`):
    var textZoom = parseString(
      attributes['${prefix}text-zoom'] ?? attributes['${prefix}textZoom'],
    );
    var withProgress = parseBool(
      attributes['${prefix}with-progress'] ??
          attributes['${prefix}withProgress'],
    );
  ...
  String toInlineProperties() {
    ...
    return [
      ...
      if (withProgress != null) 'withProgress: $withProgress',
    ].join('; ');
  }
